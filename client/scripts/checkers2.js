...
var Board = function(boardTable) {
	...
	this.handleEvent = function(e) {
		switch (e.type) {
			...
			case 'mouseup':
				...
				this.checkMove([oldY, oldX], [newY, newX]);
				break;
		}
	};
	...
	this.checkMove = function(from, to, fromEmit){
		var el = document.querySelector('div[row="' + from[0] + '"][col="' + from[1] + '"]').firstChild;
		var moveChecked = true;
		if(!fromEmit) {
			moveChecked = this.move(from, to);
			if(moveChecked) {
				server.emit('move', {from: from, to: to});
			}
		}
		// check if the piece can move
		if (moveChecked) {
			var newParent = document.querySelector('div[row="' + to[0] + '"][col="' + to[1] + '"]');
...
server.on('othermove', function(data){
	game.board.checkMove(data.from, data.to, true);
});
